...
$('#gpiosave').click(function() {
	...
	if (on !== off) {
		...
		$.post('gpiosave.php',
			...
			function(data) {
				if (data) {
					var enable1 = $('#gpio-enable').val();
					$.get('gpiostatus.php', function(status) {
						var json = $.parseJSON(status);
						if (json.pullup == 0 && enable1 == 0) {
							$.get('gpiooff.php', {devnull: 1});
							location.reload();
						} else {
							if (json.pullup == 0 && timer != $('#timer').val()) $.get('gpiotimerreset.php'); // 'on' & 'timer' changed > reset timer
							location.reload();
						}
					});
				} else {
